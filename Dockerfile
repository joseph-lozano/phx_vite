...
RUN npm install --no-audit --no-fund --prefix frontend
...
COPY priv priv
COPY --from=node /app/priv/static /app/priv/static
COPY lib lib
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/phx_vite ./
